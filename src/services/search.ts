...
export function fuseSearch(fuse: Fuse<Command>, query: string, commands: Command[], maxResults: number): SearchResult[] {
  if (!query.trim()) {
...
export function detectDirectOpen(query: string): Command | null {
  ...
  const isPath = WIN_ABS_PATH_PATTERN.test(trimmed) || UNC_PATH_PATTERN.test(trimmed) || UNIX_ABS_PATH_PATTERN.test(trimmed);
